...
function Decode({
  ...
  const [endTime, setEndTime] = useAsyncState(0);
  const [accumulatedTime, setAccumulatedTime] = useAsyncState(0);
  ...
  function handleShow() {
    stopListening();
    setEndTime(new Date()).then((res) =>
      setAccumulatedTime(accumulatedTime + differenceInSeconds(res, startTime))
    );
    setShow(true);
  }
        ...
        <button
          ...
          onClick={() => {
            if (listening) {
              ...
              setEndTime(new Date()).then((res) =>
                setAccumulatedTime(
                  accumulatedTime + differenceInSeconds(res, startTime)
                )
              );
            } else {
              startListening();
